...
import net.alpha.bttf.network.messages.MessagePlutoniumInteraction;
...
public class EntityRayTrace
{
        ...
        public static final Function<RayTraceResultRotated, EnumHand> FUNCTION_FUELING = (rayTraceResult) ->
        {
            for (EnumHand hand : EnumHand.values())
            {
                ...
                if (!stack.isEmpty() && stack.getItem() instanceof ItemPlutonium
                        && Mouse.isButtonDown(Minecraft.getMinecraft().gameSettings.keyBindUseItem.getKeyCode() + 100))
                {
                    Entity entity = rayTraceResult.entityHit;
                    if (entity instanceof EntityVehicle)
                    {
                            int fuel = ((ItemPlutonium) stack.getItem()).getCurrentPower(stack);
                            if (fuel > 0)
                            {
                                if (continuousInteractionTickCounter % 2 == 0)
                                {
                                    PacketHandler.INSTANCE.sendToServer(new MessagePlutoniumInteraction(Minecraft.getMinecraft().player, hand, entity));
                                }
                                return hand;
                            }
                        }
...
static {
    ...
    createTranformListForPart(ModItems.BODY, atvParts, atvTransformGlobal,
            MatrixTransformation.createTranslation(0, 0.7109375, 0));
    createFuelablePartTransforms(ModItems.PLUTONIUM_PLUG, 0, 0.3046875, 0, -1.57, 13.05, 5.3, new Vec3d(-90, 0, 0), 0.35, atvParts, atvTransformGlobal);
    entityRaytracePartsStatic.put(EntityCarOne.class, atvParts);
}
 ...
 * those passed to {@link net.alpha.bttf.entity.render.RenderVehicle setPartPosition} by the entity's renderer.
 *
 ...
 */
    public static void creatPartTransforms ( double xPixel, double yPixel, double zPixel, Vec3d rotation,
    double scale, List<MatrixTransformation > transforms)
    {
